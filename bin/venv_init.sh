#!/bin/bash
# Initialize the Python Virtual Environment on Ubuntu.

declare scriptName=venv_init.sh
declare version="0.1"
declare target=
...
function scriptUsage()
{
  ...
  _out
  _msg "- Initialize a python virtual environment for a debian based Linux distro."
  _out
  _out "  Args:"
  _msg "    init  : runs the initalizer and exits."
  _out
  _out "  Options:"
...
function main()
{
  ...
  REPO_DIR="$(dirname "$SCRIPTDIR")"
  venv_dir="${REPO_DIR}/.venv"
  bin_dir="${venv_dir}/bin"
  _msg "BIN_DIR: ${bin_dir}"
  if [[ ! -f  "${bin_dir}/python" ]] ; then
    _warn "missing Virtual Environment at $venv_dir"
    cd $REPO_DIR
    _cmd 'uv venv --python 3.12.0'
  fi
}
